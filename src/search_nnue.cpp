...
bool NNUE = false; // Use NNUE evaluation function
...
--------------------------------------------------------------------------------------------*/
int quiescence(Board& board, int depth, int alpha, int beta, bool leftMost) {
    ...
    for (const auto& [move, priority] : candidateMoves) {
        board.makeMove(move);
        int score = 0;
        score = -quiescence(board, depth - 1, -beta, -alpha, leftMost);
        board.unmakeMove(move);
...
int negamax(Board& board, 
    ...
    if (depth <= 0) {
        int quiescenceEval = quiescence(board, quiescenceDepth, alpha, beta, leftMost);
    ...
    if (depth <= 3 && pruningCondition && !isPV) {
        ...
        if (standPat + razorMargin < alpha) {
            ...
            return quiescence(board, quiescenceDepth, alpha, beta, leftMost);
        } 
    }
